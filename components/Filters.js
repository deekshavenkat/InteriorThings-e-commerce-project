...
const Filters = () => {
  const {filters:{text, category, company, color, min_price, max_price, price, shipping}, updateFilters, clearFilters, all_products} 
  = useFilterContext()
         ...
         <div className="form-control">
          ...
          <div>
            ...
            {colors.map((c, index)=>{
              ...
              return (
                   <button key={index} className={`${color === c ? "color-btn active": "color-btn"}`} name="color" 
              data-color ={c} style={{background :c}} onClick={updateFilters}>{color === c ? <FaCheck /> : null}</button>)
            })}
            </div>
          </div>
           {/* end of colors */}
           ...
           {/* end of price */}
           {/* // whenever we chnage price this will become string we are handling that in filter_context.js file  */}
...
// data-color ={c} get access as value in filter_context.js file 
